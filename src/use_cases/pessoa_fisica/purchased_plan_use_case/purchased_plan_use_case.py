...
class PurchasedPlanUseCase:
    ...
    def execute(self, purchased_plan_dto: PurchasedPlanDTO, response: Response, request: Request):
        ...
        now = datetime.now()
        customer_id = request.state.auth_payload.get("pessoa_fisica_id")

        if not customer_id:
            response.status_code = 400
            return {"status": "error", "message": "Customer ID (pessoa_fisica_id) is missing"}
        ...
        return {"status": "success", "message": "Plan registered successfully for PF"}
