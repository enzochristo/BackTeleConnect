...
class ResetPwdUseCase:
    ...
    def execute(self, reset_pwd_dto: ResetPwdDTO, response: Response, request: Request):
        check_exists = self.pessoa_fisica_repository.find_by_reset_pwd_token(token=reset_pwd_dto.token)
        ...
        self.pessoa_fisica_repository.update_pwd(doctor.id, reset_pwd_dto.password)

        self.pessoa_fisica_repository.update_reset_pwd_token(email=doctor.email, sent_at=0, token="")
